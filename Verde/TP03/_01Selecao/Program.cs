...
namespace _01Selecao {
  // Seleção em C#: Neste exercício você deverá ordenar os jogadores utilizando o algoritmo de seleção pelo ID do jogador.
  ...
  // 42373 Diego Alves 24/06/1985 https://tmssl.akamaized.net/images/portrait/header/42373-1543845950.jpg (614, 330)
    ...
    public static void selectionSort(Jogador[] lista) {
      for (int i=0; i < (lista.Length-1); i++) {
        if (lista[i] != null) {
          ...
          for (int j=(i+1); j < lista.Length; j++) {
            if (lista[j] != null) {
              if (lista[j].GetId() < lista[min].GetId()) 
                min = j;
            }
          }

          if (lista[i].GetId() != lista[min].GetId()) {
            Jogador aux = lista[i];
            lista[i] = lista[min];
            lista[min] = aux;
          }
